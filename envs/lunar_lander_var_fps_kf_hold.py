...
class LunarLander_VarFramerate_KF_Hold(LunarLander):
    r"""
    ## Description
    ...
    * `turbulence_power` dictates the maximum magnitude of rotational wind applied to the craft.
    ...
    Identical Kalman filter math to `envs/lunar_lander_var_fps_kf.py` (same `kf_F`/`kf_Q`/
    `kf_R`/`_kf_thrust_delta`), but the KF only ever runs (predict + update) on a
    **sampling** tick. On a **stale** tick, no KF estimation happens at all -- `kf_x`/
    `kf_P` are simply left exactly as they were after the last update, and the returned
    observation's continuous dims are the literal repeat of that last fused estimate.
    This isolates the "denoise a fresh reading via measurement fusion" benefit of the KF
    from its "predict the state forward through staleness" benefit -- compare against
    `LunarLander_VarFramerate_KF` (predicts every physics tick, sampling or not) and the
    original `LunarLander_VarFramerate` (holds the raw last *noiseless* sample, no KF at
    all) to separate out how much of the full KF's benefit comes from each piece.

    One consequence: since no predict runs during the stale gap, the single predict step
    that *does* run on the next sampling tick still only advances the state by one
    physics tick's worth of `F`/`Q` (`dt = 1/simulation_fps`), not by however many ticks
    actually elapsed since the last update -- the model doesn't attempt to account for
    the elapsed staleness at all, by design (see class docstring above).
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def _kf_thrust_delta(self, action, angle_estimate):
        ...
        only, matching this env's fixed navigation_action_space = Discrete(4). Identical
        to envs/lunar_lander_var_fps_kf.py's method of the same name.
    ...
    def step(self, action):
        ...
        if self.steps_since_last_obs >= self.obs_interval:
            # Sampling tick: the ONLY tick where the KF does any estimation at all in
            # this variant. Predict one physics tick's worth of dynamics forward from
            # the last fused estimate (NOT however many ticks actually elapsed since
            # then -- see class docstring), then fuse a fresh noisy reading via the
            # standard KF update. Thrust direction uses the KF's OWN current angle
            # estimate (index 4), not ground truth.
            ...
            # env that skips the draw entirely at sensor_noise_std=0.
            ...
            # (and the fused kf_x snapshot) across subsequent stale ticks.
            self.last_sampled_obs = obs_values.copy()

        else:
            # Stale tick: NO KF estimation happens at all here -- kf_x/kf_P are simply
            # left exactly as they were after the last update (not advanced, not
            # decayed). The returned observation's continuous dims are a literal repeat
            # of that last fused estimate -- this is the entire point of this variant,
            # vs. envs/lunar_lander_var_fps_kf.py, which keeps predicting every tick.
            self.kf_K = None
            self.kf_nis = None
            self.kf_last_z = None
...
register(
    id="LunarLander_VarFramerate_KF_Hold",
    entry_point="envs.lunar_lander_var_fps_kf_hold:LunarLander_VarFramerate_KF_Hold",
