...
import argparse
import envs.lunar_lander_var_fps_kf_hold as lunar_lander_var_fps_kf_hold
import re
...
# =========================
def evaluate_model_single_run(model, nav_model, frame_cost, budget, fixed, wind_cfg):
    env = gym.make("LunarLander_VarFramerate_KF_Hold", frame_cost=frame_cost, budget=budget, **wind_cfg)
...
if __name__ == "__main__":
    ...
    output_dir     =  "score_results/var_fps_kf_hold"
    fixed          = args.fixed
    ...
    nav_model = NavModel(args.nav_model_path, device=device)
    ...
    for run in range(N_RUNS):
        print(f"\nRun {run+1}/{N_RUNS}")
        results = evaluate_model_single_run(agent, nav_model, frame_cost, budget, fixed, wind_cfg)
    ...
    if fixed > 0:
        ...
        csv_path = os.path.join(output_dir, f"eval_results_b_{budget}_fc_{frame_cost}.csv")
    os.makedirs(output_dir, exist_ok=True)
    with open(csv_path, "w", newline="") as f:
        ...
        writer.writerow([
            ...
            args.enable_wind, args.wind_power, args.turbulence_power, args.vertical_wind_power, args.sensor_noise_std,
