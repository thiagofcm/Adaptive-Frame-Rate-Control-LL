...
import envs.lunar_lander_var_fps_kf_hold
...
class Args:
    ...
    env_id: str = "LunarLander_VarFramerate_KF_Hold"
    """the id of the environment -- the KF only predicts+updates on sampling ticks; on
    stale ticks the last fused estimate is held exactly (no predict-through-staleness).
    See envs/lunar_lander_var_fps_kf_hold.py, and compare against
    LunarLander_VarFramerate_KF (envs/lunar_lander_var_fps_kf.py), which predicts every
    physics tick, to isolate the contribution of the KF's predictive extrapolation
    during staleness from its measurement-fusion benefit on fresh ticks."""
    ...
    # Wind + Kalman-filter env parameters
    enable_wind: bool = True
    """whether the env applies random Gaussian wind/turbulence forces to the lander"""
    wind_power: float = 20.0
    """std-dev of the per-tick Gaussian horizontal wind force (also the basis for kf_Q's x/vx block)"""
    turbulence_power: float = 2.0
    """std-dev of the per-tick Gaussian torque (also the basis for kf_Q's angle/angular_velocity block)"""
    vertical_wind_power: float = 20.0
    """std-dev of the per-tick Gaussian vertical wind force (also the basis for kf_Q's y/vy block)"""
    sensor_noise_std: float = 0.05
    """std-dev of the Gaussian measurement noise added to all 6 filtered state dims on sampling ticks (kf_R)"""

def make_env(env_id, nav_model_path, frame_cost, budget, max_episode_steps,
             enable_wind, wind_power, turbulence_power, vertical_wind_power, sensor_noise_std):
    def thunk():
        nav_model = NavModel(nav_model_path, device=torch.device("cpu"))
        print("Navigation Model Loaded")
        env = gym.make(
            env_id, frame_cost=frame_cost, budget=budget,
            enable_wind=enable_wind, wind_power=wind_power,
            turbulence_power=turbulence_power, vertical_wind_power=vertical_wind_power,
...
if __name__ == "__main__":
    ...
    run_name = (f"{args.env_id}_fc_{args.frame_cost}_bud_{args.budget}_"
                f"wind{args.enable_wind}_{args.wind_power}_{args.turbulence_power}_"
                f"vert{args.vertical_wind_power}_sns{args.sensor_noise_std}_{date_str}")
    ...
    writer = SummaryWriter(f"runs/var_fps_kf_hold/{run_name}")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s" % ("\n".join([f"|{key}|{value}|" for key, value in vars(args).items()])),
    )
    info_file = os.path.join(f"runs/var_fps_kf_hold/{run_name}", "info_settings.txt")
    ...
    env_fns = [make_env(args.env_id, nav_model_path, args.frame_cost, args.budget, args.max_episode_steps,
                         args.enable_wind, args.wind_power, args.turbulence_power,
                         args.vertical_wind_power, args.sensor_noise_std)
    ...
    for iteration in range(start_iteration, args.num_iterations + 1):
        ...
        if iteration % 50 == 0:  # save every 50 iterations
            checkpoint_path = f"runs/var_fps_kf_hold/{run_name}/ckpts/timestep_{global_step}_iterations_{iteration}"
    ...
    torch.save({
        ...
        "args": vars(args),
        "global_step": global_step,
    }, f"runs/var_fps_kf_hold/{run_name}/model.pt")
    print(f"Model saved → runs/var_fps_kf_hold/{run_name}/model.pt")
    writer.close()
