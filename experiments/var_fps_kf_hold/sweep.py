#!/usr/bin/env python3
"""Sweep runner for experiments/lunar_lander_var_fps_kf_hold/train.py
(LunarLander_VarFramerate_KF_Hold -- the KF only predicts+updates on sampling ticks,
holding the last fused estimate exactly on stale ticks; compare against
experiments/var_fps_kf/, whose KF predicts every physics tick, to isolate the
contribution of predicting through staleness from measurement fusion on fresh ticks).

Reads a sweep config (default: config/sweep_lunar_lander_kf_hold.yaml) containing every
base hyperparameter (including the wind/Kalman-filter params enable_wind, wind_power,
turbulence_power, vertical_wind_power, sensor_noise_std) plus a `sweep:` section mapping
parameter names to lists of values. Builds the Cartesian product of the swept parameters,
materializes one full config file per combination, and launches train.py (this directory)
once per combination -- each pinned to its own isolated CPU core (via taskset) and
round-robined across available GPUs.
"""
import argparse
import itertools
import os
...
DEFAULT_SWEEP_CONFIG = os.path.join(REPO_ROOT, "config", "sweep_lunar_lander_kf_hold.yaml")
# train.py subprocesses run with this as cwd -- matches config/sweep_lunar_lander_kf_hold.yaml's
