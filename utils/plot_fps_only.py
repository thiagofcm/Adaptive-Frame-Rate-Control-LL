...
def evaluate(agent, nav_model, budget, frame_cost, fixed_fps,seed):
    ...
    env.close()
    return total_timesteps, fps_per_timestep, height_per_timestep, vy_per_timestep, seed
...
def plot_combined(total_timesteps, fps_per_timestep, height_per_timestep,
    ...
    fig, (ax_fps, ax_height) = plt.subplots(
        2, 1, figsize=(15, 18), sharex=True,
        gridspec_kw={"hspace": 0.08}
    )

    # # Figure background (outside plots)
    # fig.patch.set_facecolor("#FCD2AE")

    # # Plot backgrounds
    # ax_fps.set_facecolor("#FCD2AE")
    # ax_height.set_facecolor("#FCD2AE")

    ax_fps.plot(total_timesteps, fps_per_timestep, linewidth=7.0, label="Selected FPS", color="tab:blue")
    ax_fps.set_ylabel("FPS", labelpad=16)
    ax_fps.grid(True, alpha=0.9)
    for fps in FPS_CHOICES:
        ax_fps.axhline(y=fps, linestyle=":", linewidth=3.5, alpha=0.9, color="gray", label="Fixed FPS baselines" if fps == FPS_CHOICES[0] else None)
    ax_fps.set_yticks(FPS_CHOICES)
    ax_fps.set_yticklabels([str(fps) for fps in FPS_CHOICES], fontsize=40)
    ax_fps.set_ylim(0, 55)

    ax_height.plot(total_timesteps, height_per_timestep, linewidth=7.0, color="tab:orange") #label="Height")
    ...
    # )
    if fixed_fps == 0.0:
        fig.suptitle(f"Ep Seed={seed} | FC={fc} | Budget={budget}", y=0.94)
    else:
        fig.suptitle(f"Ep Seed={seed} | Fixed FPS={fixed_fps}", y=0.94)
    ...
    if landing_timestep is not None:
        ax_fps.axvline(x=landing_timestep, linestyle="--", linewidth=3.0,color="red", label="Touchdown Time")
        ax_height.axvline(x=landing_timestep, linestyle="--", linewidth=3.0, color="red", label="Touchdown Time")
...
def plot_vy_height_combined(total_timesteps, height_per_timestep, vy_per_timestep,
                  seed, budget, fc, output_dir, fixed_fps):

    landing_timestep = None
    fig, (ax_height, ax_vy) = plt.subplots(
        2, 1, figsize=(15, 14), sharex=True,
        gridspec_kw={"hspace": 0.08}
    )
    ...
    ax_vy.plot(total_timesteps, vy_per_timestep, linewidth=7.0, color="tab:purple", label="Vy")
    ax_vy.set_xlabel("Timesteps", labelpad=10)
    ax_vy.set_ylabel("Vy", labelpad=30)
    ax_vy.grid(True, alpha=0.9)
    ax_vy.axhline(y=0, linestyle="--", linewidth=1.5, color="black",
                      alpha=0.8, label="Ground (vy = 0)")
    ...
    if landing_timestep is not None:
        ax_vy.axvline(x=landing_timestep, linestyle="--", linewidth=3.0,color="red", label="Touchdown Time")
        ax_height.axvline(x=landing_timestep, linestyle="--", linewidth=3.0, color="red", label="Touchdown Time")

    #ax_vy.axvline.legend(loc="upper right")
    # Get legend entries from both subplots
    handles_vy, labels_vy = ax_vy.get_legend_handles_labels()
    handles_height, labels_height = ax_height.get_legend_handles_labels()

    # Combine them
    handles = handles_vy + handles_height
    labels = labels_vy + labels_height
    ...
    if fixed_fps == 0.0:
        filename = os.path.join(output_dir, f"vy_height_combined_seed{seed}_BUD{budget}_FC{fc}.png")
    else:
        filename = os.path.join(output_dir, f"vy_height_combined_seed{seed}_Fixed_FPS_{fixed_fps}.png")

    plt.savefig(filename, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"Saved: {filename}")

def plot_fps_only(total_timesteps, fps_per_timestep,
                  seed, budget, fc, output_dir, fixed_fps):
    ...
    for fps in FPS_CHOICES:
        ax.axhline(
            ...
            alpha=0.9,
            color="gray",
            label="Fixed FPS baselines"
            if fps == FPS_CHOICES[0] else None
        )

    ax.set_yticks(FPS_CHOICES)
    ax.set_yticklabels([str(fps) for fps in FPS_CHOICES], fontsize=55)
    ax.set_ylim(0, 55)

    if fixed_fps == 0.0:
        fig.suptitle(f"Ep Seed={seed} | FC={fc} | Budget={budget}", y=0.96)
        filename = os.path.join(
            output_dir,
            f"fps_only_seed{seed}_BUD{budget}_FC{fc}.png"
        )
    else:
        fig.suptitle(f"Ep Seed={seed} | Fixed FPS={fixed_fps}", y=0.96)
        filename = os.path.join(
            output_dir,
            f"fps_only_seed{seed}_Fixed_FPS_{fixed_fps}.png"
        )
...
def plot_vy_only(total_timesteps, vy_per_timestep,
                  seed, budget, fc, output_dir, fixed_fps):
    ...
    if fixed_fps == 0.0:
        ...
        filename = os.path.join(
            output_dir,
            f"vy_only_seed{seed}_BUD{budget}_FC{fc}.png"
        )
    else:
        fig.suptitle(f"Ep Seed={seed} | Fixed FPS={fixed_fps}", y=0.96)
        filename = os.path.join(
            output_dir,
            f"vy_only_seed{seed}_Fixed_FPS_{fixed_fps}.png"
        )
...
def plot_height_only(total_timesteps, height_per_timestep,
                     seed, budget, fc, output_dir, fixed_fps):
    ...
    ax.set_xlabel("Timesteps", labelpad=10)
    ax.set_ylabel("Height", labelpad=30)
    ...
    if fixed_fps == 0.0:
        ...
        filename = os.path.join(
            output_dir,
            f"height_only_seed{seed}_BUD{budget}_FC{fc}.png"
        )
    else:
        fig.suptitle(f"Ep Seed={seed} | Fixed FPS={fixed_fps}", y=0.96)
        filename = os.path.join(
            output_dir,
            f"height_only_seed{seed}_Fixed_FPS_{fixed_fps}.png"
        )
...
if __name__ == "__main__":
    ...
    for i in range(int(args.n_ep)):
        ...
        total_timesteps, fps_per_timestep, height_per_timestep, vy_per_timestep, seed = evaluate(
            agent=agent,
            nav_model=nav_model,
            budget=args.budget,
            frame_cost=args.fc,
        ...
        ep_results[i] = total_timesteps, fps_per_timestep, height_per_timestep, vy_per_timestep, seed

    print(f"\nPlotting {len(ep_results)} combined figures...")
    for idx in range(len(ep_results)):
        total_timesteps, fps_per_timestep, height_per_timestep, vy_per_timestep, seed = ep_results[idx]
        plot_fps_only(
        ...
        plot_vy_only(
            total_timesteps=total_timesteps,
            vy_per_timestep=vy_per_timestep,
            seed=seed,
            budget=args.budget,
            fc=args.fc,
            output_dir=args.output_dir,
